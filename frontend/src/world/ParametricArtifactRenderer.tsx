...
function getMaterialProps(
  profile: BotArtifactManifest['material_profile'],
  preview?: boolean,
  previewState?: 'valid' | 'blocked' | 'caution',
  previewValid?: boolean
) {
  if (preview) {
    const resolvedState = previewState || (previewValid ? 'valid' : 'blocked');
    let color = '#10b981'; // valid: emerald green
    if (resolvedState === 'blocked') {
      color = '#ef4444'; // blocked: rose red
    } else if (resolvedState === 'caution') {
      color = '#f59e0b'; // caution: amber orange
    }
...
// ─── Tube Artifact ────────────────────────────────────────────────────────────
function ParametricTubeArtifact({
  artifact,
  hovered,
  selected,
  preview,
  ...
  return (
    ...
    >
      ...
      <meshStandardMaterial
        {...getMaterialProps(material_profile, preview, previewState, previewValid)}
        emissiveIntensity={hovered ? material_profile.emissive_intensity + 0.5 : material_profile.emissive_intensity}
        wireframe={selected || preview}
      />
    </mesh>
  );
}
...
function InstancedClusterArtifact({
  artifact,
  hovered,
  selected,
  preview,
  ...
  return (
    ...
    >
      <Instances limit={count || 10} castShadow={!preview} receiveShadow={!preview}>
...
function LatheProfileArtifact({
  ...
  useFrame((_, delta) => {
    if (meshRef.current && !preview) meshRef.current.rotation.z += delta * 0.2;
  });
...
export function ParametricArtifactRenderer({
  artifact,
  selected,
  preview,
  previewState,
  ...
  previewState?: 'valid' | 'blocked' | 'caution';
  previewValid?: boolean;
  onInspect: (artifact: BotArtifactManifest) => void;
}) {
    ...
    <group position={transform.position} rotation={transform.rotation} scale={transform.scale}>
      {geometry_recipe.primitive_type === 'parametric_tube' && (
        <ParametricTubeArtifact
          ...
          onSelect={() => onInspect(artifact)}
        />
      )}
      {geometry_recipe.primitive_type === 'instanced_cluster' && (
        <InstancedClusterArtifact
          artifact={artifact}
          hovered={hovered}
          selected={!!selected}
          preview={preview}
      ...
      {hovered && !preview && (
        <Html distanceFactor={10} position={[0, 2, 0]} center>
          ...
          >
            ...
            <div style={{ opacity: 0.7, fontSize: 9 }}>FAMILY: {artifact.artifact_family}</div>
          </div>
        </Html>
      )}
    </group>
